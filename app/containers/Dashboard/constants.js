export const TOGGLE_MENU = 'Dashboard/TOGGLE_MENU';
export const TOGGLE_MODAL = 'Dashboard/TOGGLE_MODAL';

export const FEED_EXPERIMENTS = 'Dashboard/FEED_EXPERIMENTS';
export const FETCH_EXPERIMENTS = 'Dashboard/FETCH_EXPERIMENTS';
export const FETCH_DELETE_EXPERIMENT = 'Dashboard/FETCH_DELETE_EXPERIMENT';
export const DELETE_EXPERIMENT = 'Dashboard/DELETE_EXPERIMENT';
export const FETCH_CREATE_EXPERIMENT = 'Dashboard/FETCH_CREATE_EXPERIMENT';
export const CREATE_EXPERIMENT = 'Dashboard/CREATE_EXPERIMENT';
export const CHANGE_SELECTED_EXPERIMENT = 'Dashboard/CHANGE_SELECTED_EXPERIMENT';

export const FEED_ALGOS = 'Dashboard/FEED_ALGOS';
export const FETCH_ALGOS = 'Dashboard/FETCH_ALGOS';
export const FETCH_DELETE_ALGO = 'Dashboard/FETCH_DELETE_ALGO';
export const DELETE_ALGO = 'Dashboard/DELETE_ALGO';
export const FETCH_CREATE_ALGO = 'Dashboard/FETCH_CREATE_ALGO';
export const CREATE_ALGO = 'Dashboard/CREATE_ALGO';

export const CHANGE_FILTERS = 'Dashboard/CHANGE_SELECTED_ALGO';
export const CHANGE_SELECTED_HYPER_PARAMETER = 'Dashboard/CHANGE_SELECTED_HYPER_PARAMETER';
export const CHART_POINT_SELECT = 'Dashboard/CHART_POINT_SELECT';

export const FEED_TRIALS = 'Dashboard/FEED_TRIALS';
export const FETCH_TRIALS = 'Dashboard/FETCH_TRIALS';

export const ADD_SELECTED_METRIC = 'Dashboard/ADD_SELECTED_METRIC';
export const REMOVE_SELECTED_METRIC = 'Dashboard/REMOVE_SELECTED_METRIC';
